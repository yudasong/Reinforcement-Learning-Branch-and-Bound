...
def solve(f, input_box, output_range):
    box_stack = [input_box]
    print("This is the input box")
    print(input_box)
    print(box_stack)
    contractor = CtcFwdBwd(f, output_range)
    while box_stack:
        b = box_stack.pop()
        print("Before Contract: ",b)
        contractor.contract(b)
        print("After Contract: ",b)
        if not b.is_empty():
            branched = False
            for i in range(len(b)):
                if b[i].diam() > 0.001:
                    new_boxes = b.bisect(i, 0.4)
                    print("This is the new boxes- ",new_boxes)
...
input_box = IntervalVector(2, [0,1])
print("Hello, ",input_box)
print("The first, ",input_box[0])
#print(len(input_box))
#Define the output range (i.e. desired value of the function)
output_range = Interval(0,1)
